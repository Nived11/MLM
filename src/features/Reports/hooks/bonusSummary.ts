import { useState, useEffect } from "react";
import type { BonusSummary } from "../types";
import api from "../../../lib/api";
import { extractErrorMessages } from "../../../utils/helpers/extractErrorMessage";
import { toast } from "sonner";

interface Filters {
  username?: string;
  invoice?: string;
  limit?: number;
  offset?: number;
}

export const useBonusSummary = (filters: Filters = {}) => {
  const [users, setUsers] = useState<BonusSummary[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [totalCount, setTotalCount] = useState<number>(0);

  ////////////////// Build query string //////////////////
  const buildQuery = () => {
    return new URLSearchParams(
      Object.entries(filters).reduce((acc, [k, v]) => {
        if (v !== "" && v !== undefined && v !== null) {
          acc[k] = String(v);
        }
        return acc;
      }, {} as Record<string, string>)
    ).toString();
  };

  ////////////////// Fetch Users //////////////////
  const fetchUsers = async () => {
    try {
      setIsLoading(true);
      setError(null);

      const query = buildQuery();
      const res = await api.get(`/bonussummary-report/${query ? `?${query}` : ""}`);
      const response = res.data;

      const rawData = Array.isArray(response) ? response : response.data || [];
      const total = Array.isArray(response) ? response.length : response.total || 0;

      const mapped: BonusSummary[] = rawData.map((r: any) => ({
        id: r.id,
        username: r.username,
        invoice: r.invoice,
      }));

      setUsers(mapped);
      setTotalCount(total);
    } catch (err) {
      setError(extractErrorMessages(err) || "Could not fetch Bonus Summary");
    } finally {
      setIsLoading(false);
    }
  };

  ////////////////// Exports //////////////////
  const downloadFile = (data: Blob, filename: string) => {
    const url = window.URL.createObjectURL(new Blob([data]));
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", filename);
    document.body.appendChild(link);
    link.click();
    link.remove();
  };

  const exportPDF = async () => {
    try {
      const res = await api.get(`/bonussummary-report/?export=pdf`, { responseType: "blob" });
      downloadFile(res.data, "bonus-summary.pdf");
    } catch (err) {
      toast.error(extractErrorMessages(err));
    }
  };

  const exportExcel = async () => {
    try {
      const res = await api.get(`/bonussummary-report/?export=xlsx`, { responseType: "blob" });
      downloadFile(res.data, "bonus-summary.xlsx");
    } catch (err) {
      toast.error(extractErrorMessages(err));
    }
  };

  const exportCSV = async () => {
    try {
      const res = await api.get(`/bonussummary-report/?export=csv`, { responseType: "blob" });
      downloadFile(res.data, "bonus-summary.csv");
    } catch (err) {
      toast.error(extractErrorMessages(err));
    }
  };

  ////////////////// Copy //////////////////
  const copyToClipboard = () => {
    if (!users.length) return toast.error("No data to copy");

    const rows = [
      ["ID", "Username", "Invoice"],
      ...users.map((u) => [String(u.id), u.username, u.invoice]),
    ];

    const colWidths = rows[0].map((_, i) => Math.max(...rows.map((r) => r[i].length)));
    const formatted = rows
      .map((r) => r.map((c, i) => c.padEnd(colWidths[i] + 2)).join(""))
      .join("\n");

    navigator.clipboard.writeText(formatted);
    toast.success("Copied to clipboard!");
  };

  ////////////////// Print //////////////////
  const getPrintData = () => {
    if (!users.length) {
      toast.error("No data to print");
      return null;
    }
    return [
      ["ID", "Username", "Invoice"],
      ...users.map((u) => [String(u.id), u.username, u.invoice]),
    ];
  };

  useEffect(() => {
    fetchUsers();
  }, [JSON.stringify(filters)]);

  return {
    users,
    isLoading,
    error,
    totalCount,
    exportPDF,
    exportExcel,
    exportCSV,
    copyToClipboard,
    getPrintData,
    refetch: fetchUsers,
  };
};
