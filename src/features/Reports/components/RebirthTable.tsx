...
const RebirthTable = ({ users, isLoading, error }: Props) => {
      ...
      <table className="w-full  text-xs sm:text-sm text-white bg-black rounded-md min-w-0  sm:min-w-0">
        <thead>
          <tr className="text-left">
            ...
            <th className="px-3 py-4 whitespace-nowrap min-w-[120px]">
              STATUS
            </th>
          </tr>
        </thead>
          ...
          {isLoading ? (
            ...
            users.map((user, idx) => (
              ...
              >
                ...
                <td className="px-2 py-3">{user.sponsorname}</td>
                ...
                <td className="px-2 py-3">{user.dateofjoining}</td>
                <td className="px-2 py-3">
                  {user.status === "Active" ? (
                    <span className="px-4 py-1 text-xs font-medium rounded-full border border-gray-400 bg-gradient-to-r from-[var(--purple-2)] to-[var(--purple-1)]">
                      {user.status}
                    </span>
                  ) : (
                    <span className="px-3 py-1 text-xs font-medium rounded-full border border-gray-400 bg-transparent">
                      {user.status}
                    </span>
                  )}
                </td>
