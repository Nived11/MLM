...
const AdminNetwork = () => {
  const { data, isLoading, error } = useAdminNetwork();
  ...
  const filteredData = data.filter((user) => {
    const matchesName = user.username.toLowerCase().includes(search.toLowerCase());
    const matchesStatus = statusFilter === 'All' ? true : user.status === statusFilter;
    const matchesLevel = levelFilter === 'All' ? true : user.level === (levelFilter as number);
  ...
  return (
    <div>
      ...
      {/* Stats Cards */}
      <NetworkCard
        totalDownline={filteredData.length}
      ...
      <div className="p-4 sm:p-10">
        ...
        <div className="p-[1px] rounded-lg bg-gradient-to-r from-purple-1 to-purple-2 w-full sm:w-1/3">
          ...
          >
            <option value="All" className='text-black'>View by level</option>
            {[...new Set(data.map((u) => u.level))].map((lvl) => (
        ...
        {!isLoading && !error && (
          <>
            <div className="p-[1px] rounded-2xl bg-gradient-to-r from-purple-1  to-purple-2 mb-4">
                <div className="overflow-x-auto sm:overflow-visible rounded-2xl">
                    ...
                    <thead>
                        ...
                        <th className="py-2 px-4 border-b border-gray-700">Level</th>
                        <th className="py-2 px-4 border-b border-gray-700">Date Joined</th>
                        <th className="py-2 px-4 border-b border-gray-700">Sponsor</th>
                    ...
                    <tbody>
                        ...
                        <tr key={user.id} className="text-white text-center">
                            ...
                            <Link to={`/networks/hierarchical-view`}
                            className="text-purple-400 hover:underline hover:text-purple-300 transition">
                                {user.username}
                            </Link></td>
                            <td className="py-2 px-4 border-b border-gray-700">Level {user.level}</td>
                            <td className="py-2 px-4 border-b border-gray-700">{user.datejoined}</td>
                            <td className="py-2 px-4 border-b border-gray-700">{user.sponser}</td>
                            <td className="py-2 px-4 border-b border-gray-700">
                            <Button className='bg-gradient-to-r from-purple-1 to-purple-2 rounded-full w-30'>{user.status}</Button>
                            </td>
                        </tr>
                        ))}
                    </tbody>
            ...
            <div className="flex space-x-3 mt-6">
              <Button onClick={handleExportCSV} className='bg-gradient-to-r from-purple-1 to-purple-2 w-40 h-13'>Export CSV</Button>
              <Button onClick={handleExportPDF} className='bg-gradient-to-r from-purple-1 to-purple-2 w-40 h-13'>Export PDF</Button>
            </div>  
